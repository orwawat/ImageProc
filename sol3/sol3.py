...
# TODO - assert image resolution. assert level size. assert ndim. Test against presubmit to make sure don't need to return vectors
# TODO - Q2, Q3
# TODO - can use resize?
# TODO - finish examples
# TODO - assert all np.float32 except mask which is np.bool
# --------------------- From sol1 ------------------------
...
def blending_example1():
    """
y. Each script should display (using plt.imshow()) the two
input images, the mask, and the resulting blended image in a single figure (you can use plt.subplot()
with 4 quadrants). The examples should present color images (RGB)

Important: when you load your own images, you must use relative paths together with the this function:
def relpath(filename):
return os.path.join(os.path.dirname(__file__), filename)
For example, if you want to load the file test.jpg in the subdirectory externals of your submission,
then call:
