...
def laplacian_to_image(lpyr, filter_vec, coeff):
    """
    Rebuilds a laplacian pyramid back to a full resolution image. When coeff is all 1's,
        and filter_vec is the same like te one used to build the pyramid, the result will be exactly the
        original image.
...
def pyramid_blending(im1, im2, mask, max_levels, filter_size_im, filter_size_mask):
    ...
    :param mask: a boolean (i.e. dtype == np.bool) mask containing True and False representing which parts
                    ...
                    corresponds to 1, and False corresponds to 0. same dimensions as im1.
    :param max_levels: the max_levels parameter you should use when generating the Gaussian and Laplacian
                        pyramids.
    :param filter_size_im: the size of the Gaussian filter (an odd scalar that represents a squared filter)
                        which defining the filter used in the construction of the Laplacian pyramids of im1
                        and im2.
    :param filter_size_mask: the size of the Gaussian filter(an odd scalar that represents a squared filter)
...
def blending_example(im1pth, im2pth, maskpth, title, max_levels, filter_size_im, filter_size_mask):
    """
    Load two images and a mask, blend them, shows the result and return
    :param im1pth: relative path of first image (color)
    :param im2pth: relative path of first image (color)
    :param maskpth: relative path of the mask (boolean)
    :param title: The title of the shown figure
    :return: im1, im2, mask, im_blend
    """
    im1 = read_image(relpath(im1pth), 2)
    im2 = read_image(relpath(im2pth), 2)
    mask = load_mask(relpath(maskpth))
    im_blend = blend_rgb_image(im1, im2, mask, max_levels, filter_size_im, filter_size_mask)
    show_example(title, im1, im2, mask, im_blend)
    return im1, im2, mask, im_blend
...
def blending_example1():
    ...
    :return: im1, im2, mask, im_blend
    """
    return blending_example('externals/baby_elephant.jpg', 'externals/butterfly.jpg',
                            'externals/elephant_butterfly_mask.png', 'Example 1: Dumbo', 3, 3, 3)
...
def blending_example2():
    ...
    :return: im1, im2, mask, im_blend
    """
    return blending_example('externals/snowboarder.jpg', 'externals/dog.jpg',
                            'externals/snowboarder_dog_mask.png', 'Example 2: SnowboarDog', 6, 5, 11)
